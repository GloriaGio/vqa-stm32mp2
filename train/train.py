...
def KD_train_model(model, train_data, valid_data, config):
    ...
    callbacks.append(
        tf.keras.callbacks.EarlyStopping(monitor="val_loss", patience=5, restore_best_weights=best_weights)
    )
    '''callbacks.append(
    ...
    history = distiller.fit(
        train_data, validation_data=valid_data, epochs=config['training']['num_epochs'], callbacks=callbacks
    )
...
def scratch_train_model(model, train_data, valid_data, config):
